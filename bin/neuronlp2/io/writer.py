...
class CoNLLXWriter(object):
    ...
    def overwrite(self, sentences, comments, word, pos, head, type, lengths, symbolic_root=False, symbolic_end=False):
        ...
        for i in range(batch_size):
            ...
            for j in range(start, lengths[i] - end):
                ...
                sentence.append([j, sentences[i][j-start][1], sentences[i][j-start][2], sentences[i][j-start][3], \
                                                                                       sentences[i][j-start][4], sentences[i][j-start][5], h, t, sentences[i][j-start][8], sentences[i][j-start][9]])
            result.append((comments[i], sentence))
        return result

    def batch_write(self, datas):
        for comment, result_parsing in datas:
            self.__source_file.write("\n".join(comment))
            self.__source_file.write("\n")
            for line in result_parsing:
                self.__source_file.write("\t".join([str(x) for x in line]))
                self.__source_file.write("\n")
            self.__source_file.write("\n")

    def write(self, sentences, comments, word, pos, head, type, lengths, symbolic_root=False, symbolic_end=False):
        batch_size, _, lemma_length = word.shape
        start = 1 if symbolic_root else 0
        end = 1 if symbolic_end else 0
        for i in range(batch_size):
            ...
            for j in range(start, lengths[i] - end):
                ...
                self.__source_file.write('%d\t%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\n' % (j, sentences[i][j - start][1], sentences[i][j - start][2], sentences[i][j - start][3], \
                                                                                       sentences[i][j - start][4], sentences[i][j - start][5], h, t, sentences[i][j - start][8],
                                                                                       sentences[i][j - start][9]))
            self.__source_file.write('\n')
